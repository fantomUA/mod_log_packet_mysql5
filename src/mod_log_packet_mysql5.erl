...
%%--------------------------------------------------------------------
handle_cast({insert_packet, Id, Packet}, State) ->

	?DEBUG("Save packet: ~s", [xml:element_to_string(Packet)]),
	Query = ["INSERT INTO ", table_name(), " (msg_id, packet) VALUES", "(?, ?)"],
	Xml = xml:element_to_string(Packet),
	sql_query(Query, [Id, escape(text, Xml)]),
	{noreply, State}.
...
write_packet(From, _To, Packet, _Type, Id) ->
	...
	case Body of
		...
		_ ->
			...
			gen_server:cast(Proc, {insert_packet, Id, Packet})
	end.
...
sql_query_internal_silent(Query, Params) ->
	?DEBUG("Query sql_query_internal_silent ~s", [Query]),
